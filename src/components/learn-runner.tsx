...
function RubyHanzi({ word }: { word: Word }) {
      ...
      <div className="flex min-w-0 flex-wrap items-end gap-x-1 gap-y-2">
        {rubyParts.map((part) => (
          <span key={part.key} className="flex flex-col items-center">
            ...
            <span
              className={`font-serif ${hanziClassName} leading-tight text-zinc-900 dark:text-zinc-100`}
            >
              {part.char}
            </span>
          </span>
  ...
  return (
    <div className="min-w-0">
      <div className="break-words text-sm leading-relaxed text-zinc-500 dark:text-zinc-400">
        {word.pinyin}
      </div>
      <div
        className={`break-words font-serif ${hanziClassName} leading-tight text-zinc-900 dark:text-zinc-100`}
      >
        {word.hanzi}
      </div>
    </div>
...
function LearnView({
  ...
  return (
    <main className="flex flex-1 w-full flex-col px-4 pt-6 pb-28">
      ...
      <section className="mt-4 rounded-2xl border border-zinc-200 bg-white px-4 py-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        <div className="flex items-end justify-between gap-4">
          <RubyHanzi word={word} />
          <WordPlayer word={word} />
        </div>
        <div className="mt-4 h-px bg-zinc-200 dark:bg-zinc-800" />
        <div className="mt-4 break-words text-base leading-relaxed text-zinc-800 dark:text-zinc-200">
          {word.japanese}
        </div>
      </section>
      ...
      <div className="fixed inset-x-0 bottom-0 border-t border-zinc-200 bg-white/95 px-4 pt-3 pb-[max(env(safe-area-inset-bottom),0.75rem)] backdrop-blur dark:border-zinc-800 dark:bg-zinc-950/95">
        <div className="mx-auto flex max-w-screen-sm gap-2">
          <button
            ...
            onClick={onPrev}
            disabled={current === 1}
            className="h-14 w-24 rounded-2xl border border-zinc-300 bg-white text-sm font-semibold text-zinc-900 disabled:cursor-not-allowed disabled:opacity-30 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100"
          >
            前へ
          </button>
          <button
            type="button"
            onClick={onNext}
            className="h-14 flex-1 rounded-2xl bg-zinc-900 text-base font-semibold text-white shadow-sm active:opacity-90 dark:bg-zinc-100 dark:text-zinc-950"
          >
            {current === total ? "完了" : "次へ"}
          </button>
        </div>
...
function LearnListView({
  ...
  return (
    <main className="flex flex-1 w-full flex-col px-4 pt-6 pb-10">
      ...
      <header>
        <h1 className="text-2xl font-bold text-zinc-900 dark:text-zinc-100">{lessonTitle}</h1>
        <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">{words.length} 単語</p>
      </header>
      ...
      <section className="mt-4 flex flex-col gap-3">
        {words.map((word) => (
          <article
            ...
            className="rounded-2xl border border-zinc-200 bg-white px-4 py-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-900"
          >
            <div className="flex items-end justify-between gap-4">
              <RubyHanzi word={word} />
              <WordPlayer word={word} />
            ...
            <div className="mt-3 break-words text-base leading-relaxed text-zinc-800 dark:text-zinc-200">
              {word.japanese}
            </div>
          </article>
        ))}
  ...
  return (
    <main className="flex flex-1 w-full flex-col px-4 pt-6 pb-10">
      <header className="mb-6">
        ...
        <h1 className="mt-1 text-2xl font-bold text-zinc-900 dark:text-zinc-100">{lesson.title}</h1>
      ...
      <section className="flex flex-col gap-2">
        ...
        <Link
          href="/"
          className="flex h-12 w-full items-center justify-center rounded-2xl text-sm font-semibold text-zinc-600 dark:text-zinc-300"
        >
          トップへ戻る
        </Link>
      </section>
